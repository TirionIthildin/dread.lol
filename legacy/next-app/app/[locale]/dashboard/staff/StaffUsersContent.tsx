...
export default function StaffUsersContent() {
  ...
  const fetchUsers = useCallback(async (q?: string) => {
    try {
      const url = q?.trim()
        ? `/api/dashboard/staff/users?q=${encodeURIComponent(q.trim())}`
        : "/api/dashboard/staff/users";
      const res = await fetch(url);
      if (!res.ok) return;
      const data = await res.json();
      const list = (data.users ?? []).map((u: StaffUser & { createdAt?: Date }) => normalizeUser(u));
      setUsers(list);
      setError(null);
    } catch {
                  ...
                  <li key={u.id}>
                    ...
                    >
                      ...
                      <span className="flex-1 min-w-0">
                        ...
                        {u.slug ? (
                          <span className="block text-xs text-[var(--muted)] truncate">/{u.slug}</span>
                        ) : null}
                      </span>
                      <ChevronRight
                        size={16}
