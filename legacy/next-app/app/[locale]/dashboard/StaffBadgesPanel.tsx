...
export default function StaffBadgesPanel() {
  ...
  const fetchBadges = useCallback(async () => {
    try {
      const res = await fetch("/api/dashboard/staff/badges");
      if (!res.ok) return;
            ...
            {formBadgeType === "icon" && (
              <div className="mt-2 space-y-2">
                ...
                <p className="text-xs text-[var(--muted)]">
                  The full Lucide icon set is searchable. Names use PascalCase (e.g.{" "}
