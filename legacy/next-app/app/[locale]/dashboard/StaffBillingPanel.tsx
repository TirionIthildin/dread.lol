...
export default function StaffBillingPanel() {
  ...
  const fetchSettings = useCallback(async () => {
    try {
      const res = await fetch("/api/dashboard/staff/settings");
      if (!res.ok) return;
  ...
  const fetchProductInfo = useCallback(async (ids: string[], sandbox: boolean) => {
    ...
    try {
      const res = await fetch(
        `/api/dashboard/staff/products?ids=${encodeURIComponent(ids.join(","))}&sandbox=${sandbox}`
      );
      if (!res.ok) return {};
      const data = await res.json();
    ...
    return (
      <div className="rounded-xl border border-[var(--border)] bg-[var(--bg)]/50 overflow-hidden">
        ...
        >
          ...
          {isOpen ? (
            <ChevronDown size={16} strokeWidth={2.5} className="text-[var(--muted)]" />
          ) : (
            <ChevronRight size={16} strokeWidth={2.5} className="text-[var(--muted)]" />
          )}
        </button>
    ...
    <div className="flex-1 overflow-y-auto p-6">
      <div className="space-y-6">
            ...
            <div className="flex flex-wrap gap-2 shrink-0">
              {settings?.polarConfigured ? (
                <span className="inline-flex items-center gap-1.5 rounded-full bg-[var(--accent)]/15 px-2.5 py-1 text-xs font-medium text-[var(--accent)]">
                  <CircleCheck size={14} className="fill-current" />
                  Polar connected
                </span>
              ) : (
                <span className="inline-flex items-center gap-1.5 rounded-full bg-[var(--warning)]/15 px-2.5 py-1 text-xs font-medium text-[var(--warning)]">
                  <CircleX size={14} className="fill-current" />
        ...
        <form onSubmit={handleSave} className="space-y-4">
          {/* Setup checklist */}
          <Section id="setup" title="Setup checklist" icon={Plug} defaultOpen={false}>
