...
type AdminPanel = "users" | "improvement" | "badges";
...
export default function DashboardNavAdmin({ isAdmin, variant = "default" }: Props) {
  ...
  const buttonClass =
    variant === "sidebar"
      ? "w-full inline-flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-[var(--muted)] transition-colors hover:bg-[var(--surface-hover)] hover:text-[var(--foreground)] border border-transparent focus:outline-none focus:ring-2 focus:ring-[var(--accent)] focus:ring-offset-2 focus:ring-offset-[var(--surface)]"
