...
import { MAX_WIDGETS } from "@/app/[locale]/dashboard/profile-editor/constants";
...
export interface WidgetsSectionProps {
  ...
  setCryptoWalletEthereum: (v: string) => void;
  cryptoWalletBitcoin: string;
  setCryptoWalletBitcoin: (v: string) => void;
  cryptoWalletSolana: string;
  setCryptoWalletSolana: (v: string) => void;
...
export function WidgetsSection(props: WidgetsSectionProps) {
  const {
    ...
    cryptoWalletEthereum,
    setCryptoWalletEthereum,
    cryptoWalletBitcoin,
    setCryptoWalletBitcoin,
    cryptoWalletSolana,
    setCryptoWalletSolana,
    discordInviteInput,
      ...
      <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)]/50 overflow-hidden transition-all hover:border-[var(--border-bright)]">
        <div className="px-4 py-3 border-b border-[var(--border)] flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-2">
            <div className="rounded-lg bg-[var(--accent)]/10 p-1.5">
              <LayoutGrid size={18} className="text-[var(--accent)]" aria-hidden />
            </div>
        ...
        <div className="p-4 space-y-5">
              ...
              <>
                <div>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <WidgetCheckbox
                      ...
                      icon={<MessageCircle size={18} className="shrink-0 fill-[#5865F2] text-[#5865F2]" aria-hidden />}
                      label="Account age"
                    ...
                    <WidgetCheckbox
                      ...
                      icon={<Building2 size={18} className="shrink-0 text-[#5865F2]" aria-hidden />}
                      label="Server count"
                    ...
                    <WidgetCheckbox
                      ...
                      icon={<ExternalLink size={18} className="shrink-0 text-[#5865F2]" aria-hidden />}
                      label="Server invite"
                ...
                <div>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <WidgetCheckbox
                      ...
                      icon={<Calendar size={18} className="shrink-0 text-[var(--accent)]" aria-hidden />}
                      label="Joined"
                ...
                <div>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <WidgetCheckbox
                      ...
                      icon={<Github size={18} className="shrink-0 text-[#f0f6fc] fill-current" aria-hidden />}
                      label="Last push"
                ...
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-amber-600 dark:text-amber-400 mb-2 flex items-center gap-2">
                    <Coins size={16} strokeWidth={1.5} className="shrink-0" aria-hidden />
                    Wallet balance
                  </p>
                  <p className="text-[11px] text-[var(--muted)] mb-2">
                    Add an address per network you want to show (leave blank to skip). Each gets its own balance card. Public chain data; may be delayed. Separate from
                    the {MAX_WIDGETS} Discord/Roblox/GitHub widgets above.
                  </p>
                  <label className="block text-xs font-medium text-[var(--muted)] mb-1">
                    Ethereum address
                    <input
                      type="text"
                      name="cryptoWalletEthereum"
                      value={cryptoWalletEthereum}
                      onChange={(e) => setCryptoWalletEthereum(e.target.value)}
                      autoComplete="off"
                      spellCheck={false}
                      placeholder="0x…"
                  ...
                  <label className="block text-xs font-medium text-[var(--muted)] mb-1 mt-2">
                    ...
                    <input
                      type="text"
                      name="cryptoWalletBitcoin"
                      value={cryptoWalletBitcoin}
                      onChange={(e) => setCryptoWalletBitcoin(e.target.value)}
                      autoComplete="off"
                      spellCheck={false}
                  ...
                  <label className="block text-xs font-medium text-[var(--muted)] mb-1 mt-2">
                    ...
                    <input
                      type="text"
                      name="cryptoWalletSolana"
                      value={cryptoWalletSolana}
                      onChange={(e) => setCryptoWalletSolana(e.target.value)}
                      autoComplete="off"
                      spellCheck={false}
          ...
          {(widgetPreviewFiltered ||
            ...
            githubPreviewFiltered ||
            (cryptoPreviewData && cryptoPreviewData.length > 0)) && (
            <div className="pt-2 border-t border-[var(--border)]">
              ...
              <div className="rounded-lg border border-[var(--border)]/50 bg-[var(--bg)]/60 p-3 space-y-3">
                ...
                {cryptoPreviewData && cryptoPreviewData.length > 0 && (
                  <CryptoWidgetsDisplay wallets={cryptoPreviewData} matchAccent={widgetsMatchAccent} />
                )}
              </div>
            </div>
          )}
