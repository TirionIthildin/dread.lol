...
import type { CustomBadge } from "@/app/[locale]/dashboard/StaffBadgesPanel";
import ProfileBackground from "@/app/components/ProfileBackground";
import ProfileContent, { type ProfileVouchesData } from "@/app/components/ProfileContent";
...
type Props = {
  user: StaffUser;
  onClose: () => void;
  onUpdate: (updated: StaffUser) => void;
};

/** @deprecated Use StaffUser */
export type AdminUser = StaffUser;

export default function StaffUserModal({ user, onClose, onUpdate }: Props) {
  ...
  const [previewBundle, setPreviewBundle] = useState<{
    profile: Profile;
  ...
  useEffect(() => {
    ...
    fetch(`/api/dashboard/staff/badges?userId=${encodeURIComponent(user.id)}`)
  ...
  useEffect(() => {
    ...
    setPreviewError(null);
    fetch(`/api/dashboard/staff/users/${encodeURIComponent(user.id)}/profile-preview`)
      ...
      .then(({ ok, data }) => {
        ...
        setPreviewBundle({
          ...
          vouches: data.vouches as ProfileVouchesData,
          similarProfiles: data.similarProfiles ?? [],
          mutualGuilds: data.mutualGuilds ?? [],
        });
      })
      .catch((err: Error) => {
        if (!cancelled) setPreviewError(err.message ?? "Failed to load");
      })
      .finally(() => {
          ...
          {user.slug && (
            <div className="flex flex-wrap gap-2">
              <Link
                ...
                className="inline-flex items-center gap-1.5 rounded-lg border border-[var(--accent)]/40 bg-[var(--accent)]/10 px-3 py-1.5 text-xs font-medium text-[var(--accent)] hover:bg-[var(--accent)]/20"
              >
              ...
              <button
                ...
                className="rounded-lg border border-[var(--border)] bg-[var(--bg)]/80 px-3 py-1.5 text-xs font-medium text-[var(--foreground)] hover:bg-[var(--surface-hover)]"
              >
                Preview in modal
              </button>
            </div>
      ...
      ? createPortal(
          ...
          >
            <div
              ...
              aria-hidden
            />
            <div className="relative z-10 w-full max-w-4xl max-h-[85vh] flex flex-col rounded-xl border border-[var(--border)] bg-[var(--bg)] shadow-2xl overflow-hidden">
              <div className="flex items-center justify-between gap-2 border-b border-[var(--border)] px-3 py-2 bg-[var(--surface)]/90 shrink-0">
                <span className="text-sm font-medium text-[var(--foreground)]">Profile preview</span>
                <button
                  type="button"
                  onClick={() => setPreviewOpen(false)}
              ...
              <div className="flex-1 min-h-0 overflow-y-auto">
                {previewLoading && (
                  <p className="p-6 text-sm text-[var(--muted)] text-center">Loading preview…</p>
                )}
                {previewError && (
                  <p className="p-6 text-sm text-[var(--warning)] text-center" role="alert">
                    {previewError}
                  </p>
                )}
                {!previewLoading && previewBundle && (
                  <ProfileBackground profile={previewBundle.profile}>
                    <ProfileContent
                      profile={previewBundle.profile}
                      vouches={previewBundle.vouches}
                      similarProfiles={previewBundle.similarProfiles}
                      mutualGuilds={previewBundle.mutualGuilds}
                      canReport={false}
