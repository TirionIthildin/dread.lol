...
import {
  MAX_PROFILE_LINK_HREF_LEN,
  MAX_PROFILE_LINK_LABEL_LEN,
  parseWebsiteUrlForSave,
} from "@/lib/profile-links-display";
import { getPremiumAccess } from "@/lib/premium-permissions";
import { isValidCryptoWalletAddress, MAX_CRYPTO_WALLET_ADDRESS_LEN } from "@/lib/crypto-widgets";
...
/** User-facing errors are English for now; localize via next-intl when we add a `dashboard` namespace (see docs/i18n.md). */
...
function normalizeParsedLinkRow(x: unknown): { label: string; href: string; iconName?: string } | null {
  if (!x || typeof x !== "object") return null;
  const labelRaw = (x as { label?: unknown }).label;
  const hrefRaw = (x as { href?: unknown }).href;
  if (typeof labelRaw !== "string" || typeof hrefRaw !== "string") return null;
  const label = labelRaw.trim().slice(0, MAX_PROFILE_LINK_LABEL_LEN);
  const href = hrefRaw.trim().slice(0, MAX_PROFILE_LINK_HREF_LEN);
  if (!label || !href) return null;
  const row: { label: string; href: string; iconName?: string } = { label, href };
  const iconRaw = (x as { iconName?: unknown }).iconName;
  if (typeof iconRaw === "string" && iconRaw.trim()) {
    const lucide = resolveStoredIconNameToLucide(iconRaw.trim());
    if (lucide) row.iconName = lucide;
  }
  return row;
}
...
function parseLinksValue(linksRaw: string | null | undefined): string | null {
  ...
  if (trimmed.startsWith("[")) {
    try {
      ...
      if (!Array.isArray(arr)) return null;
      const valid = arr
        .map(normalizeParsedLinkRow)
        .filter((x): x is { label: string; href: string; iconName?: string } => x != null);
      return valid.length > 0 ? JSON.stringify(valid) : null;
    } catch {
      return null;
    }
  ...
  const parsed = trimmed
    ...
    .map((line) => {
      ...
      if (i === -1) return null;
      const label = line.slice(0, i).trim().slice(0, MAX_PROFILE_LINK_LABEL_LEN);
      const href = line.slice(i + 1).trim().slice(0, MAX_PROFILE_LINK_HREF_LEN);
...
export async function updateLinksAction(
  ...
  const copyableSocials = formData.get("copyableSocials") === "on";
  const websiteUrl = parseWebsiteUrlForSave(formData.get("websiteUrl") as string | null, copyableSocials);
  try {
    await updateMemberProfile(profileId, session.sub, {
      ...
      links: linksJson,
      socialLinksGlow: formData.get("socialLinksGlow") === "true",
    });
  } catch (e) {
  ...
  try {
    await updateMemberProfile(profileId, session.sub, {
      ...
      ...(formData.has("websiteUrl") && {
        websiteUrl: parseWebsiteUrlForSave(
          formData.get("websiteUrl") as string | null,
          formData.get("copyableSocials") === "on"
        ),
      }),
      skills: (() => {
      ...
      showDiscordAvatarDecoration: formData.get("showDiscordAvatarDecoration") === "on",
      ...
      cryptoWalletChain: null,
      cryptoWalletAddress: null,
      ...(() => {
        const slice = (name: string) =>
          (formData.get(name) as string)?.trim().slice(0, MAX_CRYPTO_WALLET_ADDRESS_LEN) ?? "";
        const eth = slice("cryptoWalletEthereum");
        const btc = slice("cryptoWalletBitcoin");
        const sol = slice("cryptoWalletSolana");
        return {
          cryptoWalletEthereum: eth && isValidCryptoWalletAddress("ethereum", eth) ? eth : null,
          cryptoWalletBitcoin: btc && isValidCryptoWalletAddress("bitcoin", btc) ? btc : null,
          cryptoWalletSolana: sol && isValidCryptoWalletAddress("solana", sol) ? sol : null,
        };
      })(),
      customFont: (() => {
