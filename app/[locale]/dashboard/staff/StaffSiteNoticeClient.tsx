...
export default function StaffSiteNoticeClient() {
  ...
  const refresh = useCallback(() => {
    setLoadError(null);
    setLoading(true);
    fetch("/api/dashboard/staff/site-notice")
  ...
  async function save(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch("/api/dashboard/staff/site-notice", {
        method: "PATCH",
  ...
  return (
    <div className="p-6 max-w-2xl space-y-6">
      <div className="flex items-start gap-3">
        <Megaphone size={28} className="shrink-0 text-[var(--accent)] mt-0.5" aria-hidden />
        <div>
