...
export default function CopyButton({ copyValue, children, ariaLabel, className = "" }: CopyButtonProps) {
  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-2 min-h-[44px] min-w-[44px] rounded-[var(--radius)] border border-[var(--border)] bg-[var(--bg)]/60 px-3 py-2.5 text-[var(--muted)] transition-colors hover:border-[var(--accent)] hover:text-[var(--accent)] hover:bg-[var(--accent)]/10 active:scale-[0.98] active:opacity-90 focus:outline-none focus:ring-2 focus:ring-[var(--accent)] focus:ring-offset-2 focus:ring-offset-[var(--surface)] ${className}`}
    >
      {children}
      {copied && (
