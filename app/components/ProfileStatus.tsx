...
export default function ProfileStatus({ status }: ProfileStatusProps) {
  return (
    <div className="mt-3 flex items-center gap-2.5 rounded-lg border border-[var(--border)]/50 bg-[var(--bg)]/50 py-2 px-3 text-xs text-[var(--muted)] w-fit max-w-full">
      <span
      ...
      <span className="font-medium min-w-0">
        <ProfileMarkdown content={status} inline />
      </span>
    </div>
  );
}
