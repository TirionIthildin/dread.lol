...
export function ProfileEditorLayout({
  ...
  return (
    <div className="flex h-[calc(100vh-11rem)] min-h-0 flex-col overflow-hidden xl:h-[calc(100vh-10rem)]">
      ...
      >
        <div className="flex">
          <TabButton active={tab === "editor"} onClick={() => setTab("editor")} icon={<Pencil size={18} strokeWidth={1.5} className="shrink-0" />}>
            Editor
          </TabButton>
          <TabButton active={tab === "preview"} onClick={() => setTab("preview")} icon={<Eye size={18} strokeWidth={1.5} className="shrink-0" />}>
            Preview
          </TabButton>
        </div>
      </nav>
        ...
        >
          <div className="border-b border-[var(--border)] px-4 py-3 flex items-center justify-between gap-2 bg-[var(--bg)]/80 shrink-0">
            <div className="flex items-center gap-2">
              <Eye size={18} strokeWidth={1.5} className="text-[var(--terminal)]" aria-hidden />
