...
import type { TemplateData, TemplateRow } from "@/lib/marketplace-templates";
...
type InitialTemplate = TemplateRow & { creatorSlug?: string | null };

export default function DashboardMarketplaceEditClient({
  templateId,
  initialTemplate,
}: {
  templateId: string;
  ...
  const id = templateId;
  const [template, setTemplate] = useState<InitialTemplate>(initialTemplate);
  const [saving, setSaving] = useState(false);
  const [name, setName] = useState(initialTemplate.name ?? "");
  const [description, setDescription] = useState(initialTemplate.description ?? "");
  const [data, setData] = useState<TemplateData>(initialTemplate.data ?? {});
  const [previewUrl, setPreviewUrl] = useState(initialTemplate.previewUrl ?? "");
          ...
          <div className="flex flex-wrap items-center gap-2">
            {previewUrl ? (
              <div className="relative w-24 h-14 rounded border border-[var(--border)] overflow-hidden bg-[var(--bg)]/50">
                <MarketplacePreviewImage src={previewUrl} alt="" fill className="object-cover" />
              </div>
            ) : null}
