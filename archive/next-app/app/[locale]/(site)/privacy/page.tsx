    ...
    <article className="relative z-10 w-full max-w-2xl mx-auto px-4 py-8">
      <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-6 shadow-sm">
        ...
        <div className="prose prose-invert prose-sm max-w-none text-[var(--foreground)] space-y-4">
          <section>
            <h2 className="text-base font-medium text-[var(--foreground)] mt-6 mb-2">{t("collect")}</h2>
            <p className="text-sm text-[var(--muted)]">{t("collectBody")}</p>
          </section>
          <section>
            <h2 className="text-base font-medium text-[var(--foreground)] mt-6 mb-2">{t("howWeUse")}</h2>
            <p className="text-sm text-[var(--muted)]">{t("howWeUseBody")}</p>
          </section>
          <section>
            <h2 className="text-base font-medium text-[var(--foreground)] mt-6 mb-2">{t("sharing")}</h2>
            <p className="text-sm text-[var(--muted)]">{t("sharingBody")}</p>
          </section>
          <section>
