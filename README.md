...
Backend-adjacent: **MongoDB**, **Valkey** (Redis-compatible sessions), **Discord OAuth**, optional **Polar** billing, file uploads on **S3-compatible storage** (production) or local disk (`FILE_STORAGE_PATH`).
...
2. **Environment** — Copy `.env.example` to `.env` and set at least:
   ...
   - `S3_BUCKET` + `S3_REGION` or `AWS_REGION` (+ credentials), or `FILE_STORAGE_PATH` (e.g. `./data/uploads`) for local dev without S3
